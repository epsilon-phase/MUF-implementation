...
void free_ast_node(struct ast_node *item) {
  ...
  }if(item->immediate)free(item->immediate);
  free(item->ordering);
  free(item);
}
...
struct ast_node *parse_function(struct tokenlist *start,
  ...
  if(ast_add_token(result,current->token)){
      eprintf("Failure to add token(this occurred when collecting the function name)\n");
  }
  current=current->next;
  if(current->next){
...
//  printf("DABOOG: %s\n",token->name);
  if(parent->immediate)
  parent->immediate = realloc(parent->immediate, sizeof(void*)*(parent->immediatecount + 1));
  else
...
#define indentTok for(int j=0;j<depth+2;j++)printf(" ");
void ast_inner_print(struct ast_node* node,int depth){
    ...
    if(depth>0)
    indent
    printf("Node:%s,%p(C:%d,T:%d)\n",buffer,(void*)node,node->childcount,node->immediatecount);
